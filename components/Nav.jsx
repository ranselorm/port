...
const Nav = () => {
  ...
  return (
    <nav className="bg-yellow-400 max-w-[1440px]">
      <ul>
        {links.map((link) => (
          <Link
            href={link.path}
            className="capitalize flex items-center flex-row w-full"
          >
            {link.name}
          </Link>
        ))}
    ...
    // <nav className="flex flex-col items-center xl:justify-center fixed gap-y-4 h-max mt-auto xl:right-[2%] z-50 xl:w-16 xl:max-w-md xl:h-screen">
